...
import (
	...
	// governancev1alpha1 "github.com/AlwaysSayNo/quorum-based-manifests-governance/cli/api/v1alpha1"
...
func (d *dummyGitRepo) GetChangedFiles(ctx context.Context, fromCommit, toCommit string, fromFolder string) ([]FileChange, error) {
	return nil, nil
}
func (d *dummyGitRepo) PushMSR(ctx context.Context, msr *ManifestSigningRequestManifestObject) (string, error) {
	return "", nil
}
func (d *dummyGitRepo) PushSignature(ctx context.Context, msr *ManifestSigningRequestManifestObject, governorAlias string, signatureData []byte) (string, error) {
	return "", nil
}
...
var _ = Describe("Repository Manager", func() {
	...
	It("should fail if RepoURL is empty", func() {
		...
		mrt := &ManifestRequestTemplate{
			ObjectMeta: metav1.ObjectMeta{
				Name:      MRTName,
				Namespace: testNamespace.Name,
			},

			Spec: ManifestRequestTemplateSpec{
				GitRepository: GitRepository{URL: TestRepoURL},
			},
		}
	...
	It("should fail if provider does not exist for the given RepoURL", func() {
		...
		mrt := &ManifestRequestTemplate{
			...
			Spec: ManifestRequestTemplateSpec{
				GitRepository: GitRepository{URL: "git@nonexitstinghub.com:/TestUser/test-repo.git"},
			},
		}
	...
	It("should fail if pgpSecrets resource does not exist", func() {
		...
		mrt := &ManifestRequestTemplate{
			...
			Spec: ManifestRequestTemplateSpec{
				PGP: &PGPPrivateKeySecret{
					SecretsRef: ManifestRef{
						Name:      PGPSecretName,
						Namespace: testNamespace.Name,
					},
