...
func (p *BaseGitProvider) GetChangedFilesRaw(
	ctx context.Context,
...
func (p *BaseGitProvider) getFileMetaInfoForPath(
	fromTree, toTree *object.Tree,
...
func (p *BaseGitProvider) GetLatestMSR(
	ctx context.Context,
...
func (p *BaseGitProvider) getMSRAndSignature(
...
func (p *BaseGitProvider) readGovernorSignatures(
	activeMSRFolderPath string,
	worktree *git.Worktree,
	policy *GovernancePolicy,
...
func (p *BaseGitProvider) GetMCAHistory(
	ctx context.Context,
	policy *GovernancePolicy,
) ([]MCAInfo, error) {
	// Sync and Lock
	...
	if err != nil {
		...
		return make([]MCAInfo, 0), nil
	}

	return p.getMCAAndSignatureList(folders, worktree, policy)
}

func (p *BaseGitProvider) getMCAAndSignatureList(
	folders []string,
	worktree *git.Worktree,
	policy *GovernancePolicy,
) ([]MCAInfo, error) {
	var mcaHistory []MCAInfo
	...
	for _, folder := range folders {
		...
		mcaHistory = append(mcaHistory, MCAInfo{
			Obj:     mca,
			Content: mcaContent,
			Sign:    mcaSignature,
		})
...
func (p *BaseGitProvider) getMCAAndSignature(
...
func (p *BaseGitProvider) getGovernanceVersionedFoldersSortedList(
	worktree *git.Worktree,
	policy *GovernancePolicy,
) ([]string, error) {
...
func (p *BaseGitProvider) getGovernanceVersionedFolders(
	worktree *git.Worktree,
	policy *GovernancePolicy,
...
func (p *BaseGitProvider) syncAndLock3(
	ctx context.Context,
...
func (p *BaseGitProvider) syncAndLock2(
	ctx context.Context,
...
func (p *BaseGitProvider) GetFileDiffPatchParts(
	ctx context.Context,
	msr *dto.ManifestSigningRequestManifestObject,
	fromCommit, toCommit string,
	...
	for _, filePatch := range patch.FilePatches() {
		...
		if msrStatus, exists := msrFCMap[path]; exists && msrStatus == status {
			selectedFilePatches[path] = NewMyFilePatch([]diff.FilePatch{filePatch}, patch.Message())
		}
	}
